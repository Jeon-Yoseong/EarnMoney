...
public class ReturnActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private List<ReturnItem> listdata;
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        toolbar.setTitle("반품수량조회");
        toolbar.setTitleTextColor(Color.parseColor("#000000"));
        setSupportActionBar(toolbar);

        // DrawerLayout 셋팅
        DrawerLayout drawer = (DrawerLayout) findViewById(R.id.return_drawer_layout);
        ...
        // NavigationView 셋팅
        NavigationView navigationView = (NavigationView) findViewById(R.id.return_nav_view);
        ...
        task.execute("http://yoseong92.cafe24.com/earnmoney/get_return_total_info.php");
    }
        ...
        protected void onPostExecute(String str) {
            try {
                ...
                for (int i = 0; i < ja.length(); i++) {
                    ...
                    listItem.add(new PHPListItem(Korea_Name, Product_Color, Size, Count, Order_Date, Return_Date, Price, Member_Id));
                }
            ...
            for (int i = listItem.size() - 1; i >= 0 ; i--) {
                if (memberId.equals(listItem.get(i).getData(7))) {
                    int count = Integer.parseInt(listItem.get(i).getData(3));
                    int price = Integer.parseInt(listItem.get(i).getData(3)) * Integer.parseInt(listItem.get(i).getData(6));
                    dateList.add(new ReturnListDateItem(listItem.get(i).getData(5), String.valueOf(count), String.valueOf(price)));
                }
            }
            ...
            if (dateList.size() != 0) {
                ...
                for(int i = 0; i < dateList.size(); i++) {
                    ...
                    if(preDate.equals(dateList.get(i).getDate())) {
                        ...
                        try {
                            listdata.add(new ReturnItem(preDate+"("+getDateDay(preDate, "yyyy/MM/dd")+")", String.valueOf(sumCount), String.valueOf(sumPrice)));
                ...
                for (int i = 0; i < totalDateList.size(); i++) {
                    System.out.println("date:"+totalDateList.get(i).getDate()+"count: "+totalDateList.get(i).getCount());
                }
                 ...
                 */
                // 어댑터에 data 넘겨줌
                adapter = new ReturnListAdapter(getApplicationContext(), listdata);
                ...
                // Recycler View 설정
                recyclerView = (RecyclerView) findViewById(R.id.return_recyclerview);
                ...
                // 데이터가 없는 요일에 0으로 초기화
                for(int i = graphSize; i < 13; i++ ) {
                    values2.add(new Entry(i - 6, 0));
                }
                // 생각해보자!!!
                ...
                // 지난주 데이터 추가
                LineDataSet lineDataSet = new LineDataSet(values, "지난주 반품수량");
                // 그래프 선 색 (빨강)
                ...
                // 이번주 데이터 추가
                LineDataSet lineDataSet2 = new LineDataSet(values2, "이번주 반품수량");
                // 파랑
                ...
                lineChart = (LineChart) findViewById(R.id.return_chart);
