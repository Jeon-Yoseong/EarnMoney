...
public class ProfitActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private List<ProfitItem> listdata;
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        // 툴바 셋팅
        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar_profit);
        toolbar.setTitle("순이익조회");
        toolbar.setTitleTextColor(Color.parseColor("#000000"));
        setSupportActionBar(toolbar);

        // DrawerLayout 셋팅
        DrawerLayout drawer = (DrawerLayout) findViewById(R.id.profit_drawer_layout);
        ...
        // NavigationView 셋팅
        NavigationView navigationView = (NavigationView) findViewById(R.id.profit_nav_view);
        ...
        protected void onPostExecute(String str) {
            try {
                ...
                for (int i = 0; i < ja.length(); i++) {
                    ...
                    listItem.add(new PHPListItem(Korea_Name, Product_Color, Size, Count, Date, Price, Korea_Unit_Cost, Member_Id));
                }
            ...
            ArrayList<ProfitListDateItem> dateList = new ArrayList<>();
            //ArrayList에 해당 판매날짜의 총수량, 총금액 저장
            ArrayList<ProfitListDateItem> totalDateList = new ArrayList<>();
            for (int i = listItem.size() - 1; i >= 0 ; i--) {
                if (listItem.get(i).getData(7).equals(memberId)) {
                    ...
                    if (count != 0) {
                        ...
                        System.out.println(listItem.get(i).getData(0)+": "+price);
                        System.out.println(listItem.get(i).getData(0)+": "+surfix);
                        System.out.println(listItem.get(i).getData(0)+": "+listItem.get(i).getData(6));
                        System.out.println(listItem.get(i).getData(0)+": "+profit);
                    }
                    dateList.add(new ProfitListDateItem(listItem.get(i).getData(4), String.valueOf(count), String.valueOf(profit)));
                }
            }
            // ArrayList 날짜 내림차순으로 정렬
            Collections.sort(dateList, new Comparator<ProfitListDateItem>() {
                @Override
                public int compare(ProfitListDateItem o1, ProfitListDateItem o2) {
            ...
            if (dateList.size() != 0) {
                ...
                for(int i = 0; i < dateList.size(); i++) {
                    ...
                    if(preDate.equals(dateList.get(i).getDate())) {
                        ...
                        try {
                            listdata.add(new ProfitItem(preDate+"("+getDateDay(preDate, "yyyy/MM/dd")+")", String.valueOf(sumCount), String.valueOf(sumProfit)));
                            totalDateList.add(new ProfitListDateItem(preDate, String.valueOf(sumCount), String.valueOf(sumProfit)));
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                ...
                for (int i = 0; i < 14; i++) {
                    ...
                    if (!totalDateList.get(j).getDate().equals(tempTodayDate)) {
                        totalDateList.add(new ProfitListDateItem(tempTodayDate, "0", "0"));
                ...
                // 날짜 내림차순으로 정렬해준다.
                Collections.sort(totalDateList, new Comparator<ProfitListDateItem>() {
                    @Override
                    public int compare(ProfitListDateItem o1, ProfitListDateItem o2) {
                        return o2.getDate().compareTo(o1.getDate());
                    }
                ...
                // 어댑터에 data 넘겨줌
                adapter = new ProfitListAdapter(getApplicationContext(), listdata);
                ...
                // Recycler View 설정
                recyclerView = (RecyclerView) findViewById(R.id.profit_recyclerview);
                ...
                for (int i = graphSize-1; i >= 0; i--) {
                    ...
                    if (i > 6) {
                        ...
                        if (countSize == totalDateListSize) {
                            ...
                            values2.add(new Entry(i - 7, Integer.parseInt(totalDateList.get(countSize).getProfit())));
                            countSize++;
                        }
                    } else {
                        // 이전 날짜의 데이터가 없는 경우 0을 넣어준다.
                        if (countSize == totalDateListSize) {
                            ...
                            values.add(new Entry(i, Integer.parseInt(totalDateList.get(countSize).getProfit())));
                            countSize++;
                        }
                    }
                }
                ...
                lineChart = (LineChart) findViewById(R.id.profit_chart);
